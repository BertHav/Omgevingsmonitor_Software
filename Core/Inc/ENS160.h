...
void ScioSense_ENS160(uint8_t slaveaddr);

void ENS_Init(I2CReadMEM readFunction, I2CWriteMEM writeFunction);				// Function to redefine I2C pins
		
bool begin(bool debug); //resets ENS160 and checks its PART_ID. Returns false on I2C problems or wrong PART_ID
bool available();			// Report availability of sensor
uint8_t revENS16x();			// Report version of sensor (0: ENS160, 1: ENS161)
...
typedef enum {
  ...
  ENS_STATE_INIT,
  ENS_STATUS_CHECK,
  ENS_STATE_START_MEASUREMENTS,
  ENS_STATE_WAIT_FOR_COMPLETION,
  ENS_STATE_PROCESS_RESULTS,
...
typedef struct {
  ...
  uint8_t _slaveaddr;             // Slave address of the ENS160
  uint16_t _stepCount;              // Counter for custom sequence
}ENS160hwsw;

//uint8_t	getMajorRev();			// Get major revision number of used firmware
//uint8_t	getMinorRev();			// Get minor revision number of used firmware
//uint8_t	getBuild();		// Get build revision number of used firmware
//uint8_t	getAQI();		// Get AQI value of last measurement
//uint16_t getTVOC();		// Get TVOC value of last measurement
//uint16_t geteCO2();		// Get eCO2 value of last measurement
//uint16_t getAQI500();		// Get AQI500 value of last measurement
//uint32_t getHP0();		// Get resistance of HP0 of last measurement
//uint32_t getHP1();		// Get resistance of HP1 of last measurement
//uint32_t getHP2();		// Get resistance of HP2 of last measurement
//uint32_t getHP3();		// Get resistance of HP3 of last measurement
//uint32_t getHP0BL();		// Get baseline resistance of HP0 of last measurement
//uint32_t getHP1BL();		// Get baseline resistance of HP1 of last measurement
//uint32_t getHP2BL();		// Get baseline resistance of HP2 of last measurement
